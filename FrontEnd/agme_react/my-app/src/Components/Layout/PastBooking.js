  ...
  async componentDidMount() {
      ...
      if (this.state.workers !== null) {
        this.state.workers.forEach((worker) => {
          axios
            ...
            .then((res) => {
              this.state.adminBookArray.push([
                { objData: res.data, wId: worker["id"] },
              ]);
            })
            .catch((err) => {
              if (err.response.status === 404) {
                this.state.adminBookArray.push([
                  { objData: null, wId: worker["id"] },
                ]);
              }
            });
        });
    ...
    try {
      ...
      if (this.state.account === "Customer") {
        const res = await axios.get("http://localhost:8080/api/bookings/past", {
          ...
          <div className="col s12 m6 offset-m3">
            <div className="card" data-test="past-bookings-card">
              ...
              <div className="card-content" data-test="account-details">
                ...
                {this.state.account !== "Admin" ? null : this.state.workers ===
                  null ? (
                ...
                {this.state.account === "Admin" ? (
                  this.state.worker === null ? (
                    <h6>
                      <b>Choose a worker to display information</b>
                    </h6>
                  ) : (
                    [
                      <h6>
                        Full name:{" "}
                        {
                          this.state.workers[this.state.worker]["user"][
                            "firstName"
                          ]
                        }{" "}
                        {
                      ...
                      <h6>
                        Service:{" "}
                        {
                          this.state.workers[this.state.worker]["service"][
                            "service"
                          ]
                        }
                      </h6>,
                  ...
                  [
                    <h6>
                      Full name: {this.state.profile["user"]["firstName"]}{" "}
                      {this.state.profile["user"]["lastName"]}
                    </h6>,
                    <h6>Email: {this.state.profile["user"]["username"]}</h6>,
                    this.state.account === "Worker" ? (
                      <h6>
                        Service: {this.state.profile["service"]["service"]}
                      </h6>
                    ) : null,
                  ]
                )}
                    ...
                    this.state.displayedBooking != null ? (
                      this.state.displayedBooking.map((book, index) => (
                        <div key={book["id"]}>
                          ...
                          <h6>
                            Date of appointment:{" "}
                            {book["startTime"].substring(0, 10)}
                          </h6>

                          <h6>
                            Customer: {book["customer"]["user"]["firstName"]}{" "}
                            {book["customer"]["user"]["lastName"]}
                          </h6>
                      ...
                      this.state.account === "Worker" ? null : (
                        this.state.book.map((book, index) => (
                          <div key={book["id"]}>
                            ...
                            <h6>
                              Worker: {book["worker"]["user"]["firstName"]}{" "}
                              {book["worker"]["user"]["lastName"]}
                            </h6>
                            <h6>Service: {book["service"]["service"]}</h6>
                            <h6>
                              Start time: {book["startTime"].substring(11)}
                            </h6>
                            <h6>End time: {book["endTime"].substring(11)}</h6>
